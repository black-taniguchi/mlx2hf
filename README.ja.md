...
- 🚀 **OpenAI互換テストサーバー**: FastAPIを使用した、`stream` 対応 of `/v1/chat/completions` エンドポイントを持つテスト用APIサーバーを同梱しています。
...
変換前に、HFとMLXの設定ファイルの差分確認・記録を行います。
```bash
...
```
*※ `config.json` に加え、`processor_config.json` や `preprocessor_config.json` がある場合は自動で検出して比較結果に含められます。*
